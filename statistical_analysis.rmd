...
analysis_data %>%
  ...
  # Calculate the percentage of male and females in each group
...
general_stats <- analysis_data %>%
  ...
  summarise(across(-c(ID, Gender, Group), list(Mean = ~round(mean(.x, na.rm = TRUE), 3), SD = ~round(sd(.x, na.rm = TRUE), 3)), .names = "{.col}_{.fn}")) %>%
  pivot_longer(
...
group_stats <- analysis_data %>%
  ...
  summarise(across(-c(ID, Gender), list(Mean = ~round(mean(.x, na.rm = TRUE), 3), SD = ~round(sd(.x, na.rm = TRUE), 3)), .names = "{.col}_{.fn}")) %>%
...
ggplot(age_educ_group, aes(x = Group, y = value, fill = Group)) +
  ...
  labs(title = "Distribution of age and education by group", y = "Value", x = "Group") +
  theme_minimal()
...
general_boxplot <- c("MMSE_Score", "FAQ_Score", "CDR_Score", "ADAS_Score", "ADAS_13_Score", "FA_global", "MD_global", "RD_global", "AxD_global")
...
# Add a filter to do the correlation study with the significant imaging variables excluding the volumes (Vol_total was not significant in the previous analysis)
image_var_sig <- general_results %>%
  filter(as.numeric(p_value) < 0.05 , variable %in% image_var, !grepl("^Vol", variable)) %>%
  pull(variable)
...
correlation_results <- expand.grid(neurocognitive_test = names(neurocognitive_var), image_analysis_results = image_var_sig, stringsAsFactors = FALSE) %>%
  rowwise() %>%
  mutate(
    correlation = cor(analysis_data[[neurocognitive_test]], analysis_data[[image_analysis_results]], use = "complete.obs"),
    p_value = cor.test(analysis_data[[neurocognitive_test]], analysis_data[[image_analysis_results]])$p.value
  ) %>%
  ungroup() %>%
  mutate(
    p_value = formatC(as.numeric(p_value), format = "e", digits = 2),
...
# Consider those results in which correlation coefficient was higher than 0.3 (moderate correlation,at least)
correlation_results_filt <- correlation_results %>% filter(abs(correlation) > 0.3)

# Display of the correlation results: correlation coefficient (r) and p-value
kable(correlation_results_filt)
```